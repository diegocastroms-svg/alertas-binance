...
cooldown_3m, cooldown_15m, cooldown_30m = {}, {}, {}
...
async def scan_tf(s, sym, tf):
    try:
        ...
        if tf in ["15m", "30m"]:
            ...
            macd_26 = ema(close, 26)
            hist = macd_12[-1] - macd_26[-1]
            if hist < 0:
                return
    ...
    async with aiohttp.ClientSession() as s:
        ...
        while True:
            try:
                ...
                symbols = sorted(symbols, key=lambda x: next((float(t["quoteVolume"]) for t in data if t["symbol"] == x), 0), reverse=True)[:100]
