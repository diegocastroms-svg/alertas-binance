...
async def scan_tf(s, sym, tf):
    try:
        ...
        ma20  = ema_last(close, 20)  if len(close)>=20  else inf  # usamos EMA20 (sua preferência por EMA)
        ...
        if tf in ("30m","1h"):
            ...
            if broke_now and book_ok:
                state[(sym,"15m")]["broke_base"] = True  # habilita sequência no 15m
        ...
        if tf == "15m" and state[key]["watch_retest"]:
            # considerar reteste apenas até 3 velas após o rompimento base
            if time.time() - state[key]["last_break_ts"] <= 3*60*60:  # margem ampla (3h) para segurança
                prev = k[-2]; curr = k[-1]
                ...
                if touched_any and resume_ok and first_green_back:
                    if can_alert(tf, sym, "retest_early"):
                        ...
                        nome = sym.replace("USDT", "")
                        msg = (
                            f"<b>📘 RETESTE ANTECIPADO DETECTADO (15M)</b>\n\n"
                            f"{nome}\n\n"
        ...
        # cancelamento do ciclo (se perder força)
...
async def main_loop():
    async with aiohttp.ClientSession() as s:
        await tg(s, "<b>V7.3 ATIVO — TENDÊNCIA CURTA (Entrada Antecipada + Reteste Antecipado)</b>\n15m/30m/1h | Vol24≥10M | Cooldown 15m | Scan 30s")
        while True:
            try:
