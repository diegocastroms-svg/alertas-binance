...
# 4) [ADIÇÃO] Novos alertas longos independentes (cooldown individual):
...
class Monitor:
    def __init__(self):
        ...
        self.cooldowns_long = defaultdict(lambda: defaultdict(lambda: 0.0))  # longos por tipo
        self.rs_24h = {}
        self.btc_pct = 0.0
...
async def longterm_worker(session, symbol, monitor: Monitor):
    ...
    - [ADIÇÃO] Novos alertas independentes:
        ...
        * PRÉ-CONFIRMAÇÃO (4H)
        * TENDÊNCIA 4H CONFIRMADA
    Cooldown: 1h por ativo e por TIPO de alerta longo (independente).
    """
    try:
        # 15m
        ...
        if cond_15 and cond_1 and cond_4:
            if monitor.allowed_long(symbol, "LONG_COMBINED"):
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            tasks = []
            for s in watchlist:
                tasks.append(candle_worker(session, s, monitor))   # curto
                tasks.append(longterm_worker(session, s, monitor)) # longo
