# main_py15c.py — Auditoria e correções:
# - Reintegra longterm_worker (1h/4h) igual ao py base
# - Corrige ADX (real) e usa ADX >= 25.0 onde precisa
# - Adiciona alertas curtos que faltavam: Pré 5m, Pré 15m, Confirm 15m
# - Inclui Resistência 15m e Reteste 15m
# - Mantém reteste removido no 5m (como você pediu)
# - Flask no final para manter ativo no Render
...
# ADX (real, robusto o suficiente p/ decisão)
...
def shortlist_from_24h(tickers, n=400):
    ...
    for t in tickers:
        ...
        if not s.endswith("USDT"): 
            continue
        # SPOT-only (bloqueia alavancados/perp/fiats alternativos)
...
async def candle_worker(session, symbol, monitor: Monitor):
    try:
        ...
        # 🚀 Tendência iniciando (5m) — EMA9 cruza acima de MA20/MA50 + RSI>55
        ...
        # 🌕 Pré-confirmada 5m — 9/20/50 > 200 e (MA20 OU MA50) acabou de cruzar a 200
        crossed_20 = (ma20[last] > ma200[last] and ma20[last-1] <= ma200[last-1])
        crossed_50 = (ma50[last] > ma200[last] and ma50[last-1] <= ma200[last-1])
        if (ema9[last] > ma20[last] > ma50[last] > ma200[last] and (crossed_20 or crossed_50)):
            if monitor.allowed(symbol, "PRECONFIRM_5M"):
                txt = (f"🌕 {fmt_symbol(symbol)} — TENDÊNCIA PRÉ-CONFIRMADA (5m)\n"
                       f"💰 <code>{c[last]:.6f}</code>\n"
                       f"🧠 Médias 9, 20 e 50 acima da 200 | (cruzamento recente de 20/50 > 200)\n"
                       f"⏰ {ts_brazil_now()}\n{binance_links(symbol)}")
        ...
        # 🚀 Confirmada 15m — MA20 e MA50 > MA200 + RSI>55 + ADX >= 25
        if (ma20_15[last15] > ma200_15[last15] and ma50_15[last15] > ma200_15[last15] and
            ...
            if monitor.allowed(symbol, "CONFIRM_15M"):
                txt = (f"🚀 {fmt_symbol(symbol)} — TENDÊNCIA CONFIRMADA (15m)\n"
                       f"💰 <code>{c15[last15]:.6f}</code>\n"
                       f"🧠 MA20 e MA50 acima da MA200 | RSI {rsi15[last15]:.1f} | ADX {adx15[last15]:.1f}\n"
                       f"⏰ {ts_brazil_now()}\n{binance_links(symbol)}")
                await send_alert(session, txt); monitor.mark(symbol, "CONFIRM_15M")

        # 📈 Rompimento de resistência (15m) — Donchian 20-high
        if last15 >= 21:
            donchian_high_15 = max(h15[last15-20:last15])
            if c15[last15] > donchian_high_15 and monitor.allowed(symbol, "BREAKOUT_15M"):
                txt = (f"⭐ {fmt_symbol(symbol)} 📈 — ROMPIMENTO DA RESISTÊNCIA (15m)\n"
                       f"💰 <code>{c15[last15]:.6f}</code>\n"
                       f"🧠 Fechou acima da máxima 20 ({donchian_high_15:.6f}) — 💥 Rompimento confirmado\n"
                       f"⏰ {ts_brazil_now()}\n{binance_links(symbol)}")
                await send_alert(session, txt); monitor.mark(symbol, "BREAKOUT_15M")

        # ♻️ Reteste 15m — toque EMA9/MA20 + reação (com tendência curta já alinhada)
        touched_ema9 = (l15[last15] <= ema9_15[last15] and c15[last15] >= ema9_15[last15])
        touched_ma20 = (l15[last15] <= ma20_15[last15] and c15[last15] >= ma20_15[last15])
        if (ema9_15[last15] > ma20_15[last15] > ma50_15[last15] and (touched_ema9 or touched_ma20)):
            if monitor.allowed(symbol, "RETESTE_15M"):
                base = "EMA9" if touched_ema9 else "MA20"
                txt = (f"⭐ {fmt_symbol(symbol)} ♻️ — RETESTE {base} (15m)\n"
                       f"💰 <code>{c15[last15]:.6f}</code>\n"
                       f"🧠 Toque na {base} + reação — Continuação da alta\n"
                       f"⏰ {ts_brazil_now()}\n{binance_links(symbol)}")
                await send_alert(session, txt); monitor.mark(symbol, "RETESTE_15M")

    except Exception as e:
        print("worker curto error", symbol, e)

# ----------------- WORKER LONGO (1h + 4h) — igual ao py base -----------------
async def longterm_worker(session, symbol, monitor: Monitor):
    try:
        ...
        if (ema9_1[last1] > ma20_1[last1] and ema9_1[last1-1] <= ma20_1[last1-1] and
            50.0 <= rsi1[last1] <= 60.0 and v1[last1] >= volma1[last1] and monitor.allowed_long(symbol)):
            ...
            await send_alert(session, txt); monitor.mark_long(symbol); return

        # 🚀 Tendência Longa Confirmada (1h): 9>20>50 + RSI>55 + ADX≥25
        if (ema9_1[last1] > ma20_1[last1] > ma50_1[last1] and rsi1[last1] > 55.0 and adx1[last1] >= 25.0 and monitor.allowed_long(symbol)):
        ...
        if (ema9_4[last4] > ma20_4[last4] and ema9_4[last4-1] <= ma20_4[last4-1] and rsi4[last4] > 50.0 and monitor.allowed_long(symbol)):
        ...
        # 🚀 Tendência 4h Confirmada: 9>20>50 por 2 velas + RSI>55
        if (ema9_4[last4] > ma20_4[last4] > ma50_4[last4] and
            ema9_4[last4-1] > ma20_4[last4-1] > ma50_4[last4-1] and
            rsi4[last4] > 55.0 and monitor.allowed_long(symbol)):
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        hello = f"💻 py15c | Curto 5m/15m OK + Longo 1h/4h reativado | {len(watchlist)} pares | {ts_brazil_now()}"
        ...
        while True:
            ...
            # refresh watchlist a cada ciclo
            try:
                tickers = await get_24h(session)
                watchlist = shortlist_from_24h(tickers, SHORTLIST_N)
            except Exception as e:
...
@app.route("/")
def home():
    return "✅ Binance Alerts Bot py15c — Ativo e monitorando (5m/15m/1h/4h) 🇧🇷"
