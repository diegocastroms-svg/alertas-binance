# ============================================
# 📁 main_v2_3_dynamic.py
# ============================================
# Binance Spot Alerts (v2.3) — Dynamic Volume Scanner:
# - Varre TODAS SPOT/USDT e seleciona TOP 50 por volume 24h, auto-update 1h
# - 3 setups em paralelo:
#   1) Pump (5m + check 15m) — fases: formação / entrada segura / saída
#   2) Day (15m) — reteste inteligente (entrada possível / segura / saída)
#   3) Swing (1h/4h) — confirmação multi-TF (entrada possível / segura / saída)
...
@app.route("/health")
def health():
    return "OK — v2.3 dynamic (pump/day/swing)", 200
...
async def compute_top50(session):
    ...
    for t in tickers:
        ...
        if any(x in base for x in ANTI_LIST):
            continue
        # quoteVolume pode vir como string
        try:
...
# -----------------------------
# 🧠 Regras de classificação (entrada/saída)
# -----------------------------
...
# -----------------------------
async def main_loop():
    print("✅ Bot iniciado no Render v2.3 Dynamic Scanner")
