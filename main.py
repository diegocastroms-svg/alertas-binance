...
INTERVAL = "15m"                # alterado de 5m -> 15m
SHORTLIST_N = 40
COOLDOWN_SEC = 15 * 60
MIN_PCT = 1.0
MIN_QV = 300_000.0
...
def compute_indicators(open_, high, low, close, volume):
    ...
    return ema9, ma20, ma50, rsi14, vol_ma, hh20, res20, res50

# --------------- Regras ---------------
def check_signals(close, high, low, volume, ema9, ma20, ma50, rsi14, vol_ma, hh20, res20, res50):
    n = len(close)
    if n < 60: return []
    last, prev = n - 1, n - 2
    out = []
...
def pick_priority_kind(signals):
    prio = {
        "PUMP":0,"BREAKOUT":1,"REVERSÃO":2,"RETESTE":3,
        "RESISTÊNCIA_CURTA":4,"RESISTÊNCIA_LONGA":5,"TENDÊNCIA":6
    }
...
async def candle_worker(session, symbol: str, monitor: Monitor):
    try:
        ...
        ema9, ma20, ma50, rsi14, vol_ma, hh20, res20, res50 = compute_indicators(open_, high, low, close, volume)
        signals = check_signals(close, high, low, volume, ema9, ma20, ma50, rsi14, vol_ma, hh20, res20, res50)
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            ...
            await asyncio.sleep(600)  # ajustado para 15m timeframe
            try:
                tickers = await get_24h(session)
                watchlist = shortlist_from_24h(tickers, SHORTLIST_N)
            except Exception as e:
...
if __name__ == "__main__":
    ...
    @app.route("/")
    def home():
        return "✅ Binance Alerts Bot (15m + Resistência) ativo!"
