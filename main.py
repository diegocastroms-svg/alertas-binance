# main.py — V7.3 OURO CONFLUÊNCIA REAL — TENDÊNCIA CURTA (Entrada Antecipada Real + DEBUG)
# Loga moedas escaneadas e mantém alertas de início de rompimento
...
@app.route("/")
def home():
    return "V7.3 OURO CONFLUÊNCIA REAL — TENDÊNCIA CURTA (Entrada Antecipada Real + DEBUG) ATIVO", 200
...
def macd_virando(close):
    if len(close) < 26: return False, 0
    e12 = ema(close, 12)
    e26 = ema(close, 26)
    macd_line = e12[-1] - e26[-1]
    hist = macd_line - ema([a - b for a, b in zip(e12, e26)], 9)[-1]
    hist_prev = macd_line - ema([a - b for a, b in zip(e12[:-1], e26[:-1])], 9)[-1]
    return hist > hist_prev, hist

def vol_strength(vol):
    if len(vol) < 21: return 100
    ma9 = sum(vol[-9:]) / 9
    ma21 = sum(vol[-21:]) / 21
    base = (ma9 + ma21) / 2 or 1e-12
    return (vol[-1] / base) * 100
...
async def scan_tf(s, sym, tf):
    try:
        ...
        taker_sell = float(t.get("quoteVolume", 0)) - taker_buy
        book_ok = (taker_buy >= taker_sell * BOOK_DOM) or (taker_buy == 0)

        nome = sym.replace("USDT", "")

        # Logar análise no Render
        print(f"[{now_br()}] {tf.upper()} {nome} | RSI {r:.1f} | Vol {vs:.0f}% | MACD {'↑' if hist_up else '↓'} | Book {'OK' if book_ok else 'fraco'}")

        # --- ALERTA DE ENTRADA ANTECIPADA REAL ---
        if (
            price > ema200 * 0.995 and  # início do rompimento
            hist_up and
            45 <= r <= 60 and
            vs >= 100 and
            book_ok and
